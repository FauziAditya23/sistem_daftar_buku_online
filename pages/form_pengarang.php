    ...
    try {
        ...
        if (isset($_POST['action'])) {
            ...
            if ($action === 'add') {
                ...
                $stmt = mysqli_prepare($conn, "SELECT pengarang_id FROM tbl_pengarang WHERE
pengarang_id = ?");
                ...
                // Insert new author
                $stmt = mysqli_prepare($conn, "INSERT INTO tbl_pengarang (pengarang_id,
pengarang_nama) VALUES (?, ?)");
                mysqli_stmt_bind_param($stmt, 'ss', $pengarang_id, $pengarang_nama);
                if (mysqli_stmt_execute($stmt)) {
                    $message = 'Pengarang berhasil ditambahkan.';
                    $message_type = 'success';
                    header('Location: index.php?page=kelola_pengarang&message=' .
                        urlencode($message) . '&type=' . $message_type);
                    exit;
                } else {
                    throw new Exception('Gagal menambahkan pengarang: ' .
                        mysqli_error($conn));
                }
            } elseif ($action === 'edit') {
                // Edit author
                ...
                $stmt = mysqli_prepare($conn, "UPDATE tbl_pengarang SET pengarang_nama = ?
WHERE pengarang_id = ?");
                ...
                if (mysqli_stmt_execute($stmt)) {
                    ...
                    throw new Exception('Gagal mengupdate pengarang: ' .
                        mysqli_error($conn));
                }
            }
            if (isset($stmt)) {
...
<head>
    ...
    <title><?php echo $action === 'edit' ? 'Edit' : 'Tambah'; ?> Pengarang - <?php echo
                  htmlspecialchars($user_level); ?></title>
    ...
    <div class="form-container">
        ...
        <?php elseif ($action === 'edit' && $edit_data): ?>
            <form method="post">
                ...
                <div class="form-group">
                    <label for="pengarang_id">ID Pengarang:</label>
                    <input type="text" id="pengarang_id" name="pengarang_id" value="<?php echo
                        htmlspecialchars($edit_data['pengarang_id']); ?>" readonly required>
                </div>
                <div class="form-group">
                    <label for="pengarang_nama">Nama Pengarang:</label>
                    <input type="text" id="pengarang_nama" name="pengarang_nama" value="<?php
                    echo htmlspecialchars($edit_data['pengarang_nama']); ?>" required maxlength="150">
                </div>
