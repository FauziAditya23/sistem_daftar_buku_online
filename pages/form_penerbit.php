        ...
        if (isset($_POST['action'])) {
            ...
            if ($action === 'add') {
                ...
                if (mysqli_stmt_execute($stmt)) {
                    ...
                    header('Location: index.php?page=kelola_penerbit&message=' .
                        urlencode($message) . '&type=' . $message_type);
                    exit;
                } else {
                    throw new Exception('Gagal menambahkan penerbit: ' .
                        mysqli_error($conn));
                }
            } elseif ($action === 'edit') {
                // Edit publisher
                ...
                $stmt = mysqli_prepare($conn, "UPDATE tbl_penerbit SET penerbit_nama = ?
WHERE penerbit_id = ?");
                ...
                if (mysqli_stmt_execute($stmt)) {
                    ...
                    throw new Exception('Gagal mengupdate penerbit: ' .
                        mysqli_error($conn));
                }
            }
            if (isset($stmt)) {
...
<head>
    ...
    <title><?php echo $action === 'edit' ? 'Edit' : 'Tambah'; ?> Penerbit - <?php echo
    ...
    <div class="form-container">
        ...
        <?php if ($action === 'add'): ?>
            <form method="post">
                ...
                <div class="form-group"><label for="penerbit_nama">Nama Penerbit:</label>
        ...
        <?php elseif ($action === 'edit' && $edit_data): ?>
            <form method="post">
                ...
                <div class="form-group">
                    <label for="penerbit_id">ID Penerbit:</label>
                    <input type="text" id="penerbit_id" name="penerbit_id" value="<?php echo
                        htmlspecialchars($edit_data['penerbit_id']); ?>" readonly required>
                </div>
                <div class="form-group">
                    <label for="penerbit_nama">Nama Penerbit:</label>
                    <input type="text" id="penerbit_nama" name="penerbit_nama" value="<?php
                    echo htmlspecialchars($edit_data['penerbit_nama']); ?>" required maxlength="150">
                </div>
