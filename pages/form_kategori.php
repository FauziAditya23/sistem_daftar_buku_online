    ...
    try {
        ...
        if (isset($_POST['action'])) {
            ...
            if ($action === 'add') {
                ...
                $stmt = mysqli_prepare($conn, "SELECT kategori_id FROM tbl_kategori WHERE
kategori_id = ?");
                ...
                // Insert new category
                $stmt = mysqli_prepare($conn, "INSERT INTO tbl_kategori (kategori_id,
kategori_nama) VALUES (?, ?)");
                mysqli_stmt_bind_param($stmt, 'ss', $kategori_id, $kategori_nama);
                if (mysqli_stmt_execute($stmt)) {
                    $message = 'Kategori berhasil ditambahkan.';
                    $message_type = 'success';
                    header('Location: index.php?page=kelola_kategori&message=' .
                ...
                $stmt = mysqli_prepare($conn, "UPDATE tbl_kategori SET kategori_nama = ?
WHERE kategori_id = ?");
...
<head>
    ...
    <title><?php echo $action === 'edit' ? 'Edit' : 'Tambah'; ?> Kategori - <?php echo
    ...
    <div class="form-container">
        ...
        <?php elseif ($action === 'edit' && $edit_data): ?>
            <form method="post">
                ...
                <div class="form-group">
                    <label for="kategori_id">ID Kategori:</label>
                    <input type="text" id="kategori_id" name="kategori_id" value="<?php echo
                        htmlspecialchars($edit_data['kategori_id']); ?>" readonly required>
                </div>
                <div class="form-group">
                    <label for="kategori_nama">Nama Kategori:</label>
                    <input type="text" id="kategori_nama" name="kategori_nama" value="<?php
                    echo htmlspecialchars($edit_data['kategori_nama']); ?>" required maxlength="150">
                </div>
