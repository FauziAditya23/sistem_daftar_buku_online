...
<body>
    <div class="crud-container">
        <div class="crud-header">
            <h1>Kelola Buku</h1>
            <div>
                <a href="index.php?page=dashboard" class="btn btn-secondary">← Kembali ke
                    Dashboard</a>
                <a href="index.php?page=form_buku&action=add" class="btn btn-primary">Tambah
                    Buku</a>
            </div>
        </div>
                ...
                <?php else: ?>
                    <?php foreach ($books as $buku): ?>
                        <tr>
                            ...
                            <td>
                                <?php echo htmlspecialchars($buku['kategori_nama'] ?? '-');
                                ?>
                            </td>
                            <td>
                                <?php echo htmlspecialchars($buku['pengarang_nama'] ?? '-');
                                ?>
                            </td>
                            <td>Rp <?php echo
                                htmlspecialchars(number_format($buku['buku_harga'], 0, ',', '.')); ?></td>
                            <td>
                                <a href="index.php?page=form_buku&action=edit&id=<?php echo
                                    htmlspecialchars($buku['buku_isbn']); ?>" class="btn btn-success">Edit</a>
                                <button onclick="deleteBook('<?php echo
                                    htmlspecialchars($buku['buku_isbn']); ?>')" class="btn btn-danger">Hapus</button>
                            </td>
                            <td>
                                <button onclick="openDetailModal('<?php echo
                                    htmlspecialchars($buku['buku_isbn']); ?>')" class="btn btn-primary">Detail</button>
                            </td>
                        </tr>
                    <?php endforeach; ?>
                <?php endif; ?>
    ...
    <div id="deleteModal" class="modal">
        <div class="modal-content">
            ...
            <form method="post" id="deleteForm">
                ...
                <button type="button" onclick="closeModal('deleteModal')" class="btn btnsecondary">Batal</button>
            </form>
        </div>
    </div>
    <!-- Detail Book Modal -->
    <div id="detailModal" class="modal">
        <div class="modal-content">
            ...
            <button type="button" onclick="closeModal('detailModal')" class="btn btn-secondary"
                style="margin-top: 20px;">Tutup</button>
        </div>
    </div>
    <script>
 ...
 <div class="detail-value"><?php echo
           htmlspecialchars($book_detail['buku_isbn'] ?? '-'); ?></div>
 </div>
 <div class="detail-row">
 <div class="detail-label">Judul Buku:</div>
 <div class="detail-value"><?php echo
           htmlspecialchars($book_detail['buku_judul'] ?? '-'); ?></div>
 </div>
 <div class="detail-row">
 <div class="detail-label">Penerbit:</div>
 <div class="detail-value"><?php echo
           htmlspecialchars($book_detail['penerbit_nama'] ?? '-'); ?></div>
 </div>
 <div class="detail-row">
 <div class="detail-label">Kategori:</div>
 <div class="detail-value"><?php echo
           htmlspecialchars($book_detail['kategori_nama'] ?? '-'); ?></div>
 </div>
 <div class="detail-row">
 <div class="detail-label">Pengarang:</div>
 <div class="detail-value"><?php echo
           htmlspecialchars($book_detail['pengarang_nama'] ?? '-'); ?></div>
 </div>
 <div class="detail-row">
 ...
 <div class="detail-value"><?php
       ...
       if (!empty($date_value) && $date_value !== '0000-00-00') {
           ...
           if (count($date_parts) === 3) {
               echo htmlspecialchars($date_parts[2] . '/' . $date_parts[1] .
                   '/' . $date_parts[0]);
           } else {
 ...
 <div class="detail-value"><?php echo
           htmlspecialchars($book_detail['buku_jmlhalaman'] ?? '-'); ?></div>
 </div>
 <div class="detail-row">
 <div class="detail-label">Harga:</div>
 <div class="detail-value">Rp <?php echo
           htmlspecialchars(number_format($book_detail['buku_harga'] ?? 0, 0, ',', '.')); ?></div>
 </div>
 <div class="detail-row">
 <div class="detail-label">Deskripsi:</div>
 <div class="detail-value"><?php echo
           nl2br(htmlspecialchars($book_detail['buku_deskripsi'] ?? '-')); ?></div>
 </div>
 `;
